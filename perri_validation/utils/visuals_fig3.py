...
from perri_validation.utils.clinical.battery import add_battery_column, battery_for_test_code
from perri_validation.constants.fig_config import A4_WIDTH, FONT_SIZE_PANEL_TITLE, FONT_SIZE_TICK_LABEL, MODEL2COLOR, REF_LINE_STYLE
from perri_validation.constants.marker_config import BATTERY2TESTCODE, TESTCODE_DISPLAY
from perri_validation.constants.runtime import CV_COL, MU, TEST_CODE_COL
from perri_validation.utils.visuals_shared import FIG3_CAPSIZE, FIG3_ERROR_BAR_MARKERSIZE, OFFSET, _snap_yticks_to_ylim, format_battery_label, save_fig_as_svg
...
def battery_tests_in_ioi_order(df: pd.DataFrame, battery: str, ioi_order: list, test_col: str = TEST_CODE_COL) -> list:
    """Return tests in a battery, highest-`ioi` first (see marker_config.MARKER_IOI_ORDER).
    ...
    ordered = [tc for tc in reversed(ioi_order) if tc in present]
